...
// initialize object
$customer_count = new admin($db);
  
// read products will be here
$data = json_decode(file_get_contents("php://input"));
//print_r($data);
$customer_count->userId=$data->UserId;
...
$stmt = $customer_count->customerCount();
$num = $stmt->rowCount();
...
if($num>0){
    ...
    // products array
    $customer_count_arr=array();
    $customer_count_arr["records"]=array();
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $customer_count_item=array(

            "customer_count" => $customer_count
            
        );
  
        array_push($customer_count_arr["records"], $customer_count_item);
    }
    ...
    echo json_encode($customer_count_arr);
}
...
// no products found will be here
    ...
    echo json_encode(
        array("message" => "users customer count not found")
    );
