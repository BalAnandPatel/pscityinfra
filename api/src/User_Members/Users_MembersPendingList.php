...
$stmt= $MembersApprovedDetails->Users_MembersPendingList();
$num= $stmt->rowCount();
if($num>0){
...
while($row= $stmt->fetch(PDO::FETCH_ASSOC)){
   ...
   echo json_encode(
       array("message" => "User's Members pending list not found.")
   );
   $log_msg="User's Members pending list not found : " . basename($_SERVER['PHP_SELF']);
   logger($log_msg);
}
}
catch (Exception $e){
