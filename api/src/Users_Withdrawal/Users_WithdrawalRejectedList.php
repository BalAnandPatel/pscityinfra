...
$UsersRejectedList->WithdrawalStatus= $data->WithdrawalStatus; 
...
$stmt = $UsersRejectedList->Users_WithdrawalPendingList();
// query products
...
if($num>0 ){
    ...
    $Withdrawal_read_array["WithdrawalApprovedList"]=array();

    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        extract($row);
      echo  $Withdrawal_Obj=array(
        ...
        array_push($Withdrawal_read_array["WithdrawalApprovedList"], $Withdrawal_Obj);
    }
    ...
    echo json_encode(
        array("message" => "User Withdrwal Rejected List not Found.")
    );
    $log_msg="User Withdrwal  Rejected List not Found. : " . basename($_SERVER['PHP_SELF']);
    logger($log_msg);
 }
 }
 catch (Exception $e){
